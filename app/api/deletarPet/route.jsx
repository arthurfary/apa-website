...
export async function DELETE(request) {
    ...
    if (!id) {
        return NextResponse.json({ message: "ID do pet não fornecido!", success: 0 }, { status: 400 });
    }
    ...
    try {
        ...
        if (result.rowCount === 0) {
            return NextResponse.json({ message: "Pet não encontrado!", success: 0 }, { status: 404 });
        }

        // Retorna sucesso
        return NextResponse.json({ message: "Pet deletado com sucesso!", success: 1 }, { status: 200 });
    } catch (error) {
        // Trata possíveis erros
        console.error(error);
        return NextResponse.json({ message: "Erro ao deletar o pet.", success: 0 }, { status: 500 });
    }
}
